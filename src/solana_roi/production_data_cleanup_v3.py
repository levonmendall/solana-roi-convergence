...
"""Exact-dependency production cleanup executor.

V3 keeps the conservative V2 deletion predicates but fixes certifier truth to the
actual durable replica sidecar contract and adds invariant snapshots around every
protected table, the paper-engine checkpoint, and the append-only event head.
Unknown tables remain protected automatically.
"""
...
CLEANUP_VERSION = "production-data-cleanup-v3"
...
# Exact current canonical names plus the V2 compatibility names. Unknown tables are
# protected regardless, so this set is documentary as well as mechanically explicit.
...
PREDICATE_TABLES = base.PREDICATE_TABLES
...
def _read_certifier_state(database_path: Path) -> dict[str, Any]:
    ...
    if not str(payload.get("release_commit") or "").strip():
        raise CleanupBlocked("certifier durable replica release is empty")
    if not str(payload.get("epoch") or "").strip():
        raise CleanupBlocked("certifier durable replica epoch is empty")
    if not str(payload.get("schema_fingerprint") or "").strip():
        raise CleanupBlocked("certifier durable replica schema fingerprint is empty")
    ...
    return {
        ...
        "bootstrap_complete": bool(payload.get("bootstrap_complete")),
...
def _event_head_snapshot(
    ...
    head = db.execute(
        "SELECT id,event_type,observed_at,previous_hash,lineage_hash FROM events ORDER BY id DESC LIMIT 1"
    ).fetchone()
    return {
...
def execute_cleanup(
    ...
    *,
    ...
    try:
        # The cross-process lease is the outer production invariant. BEGIN IMMEDIATE
        # independently proves that no uncoordinated SQLite writer still owns the DB.
        ...
        report["durable_source_watermark"] = (
            int(certifier_state_before["watermark"]) if certifier_state_before else None
        )
        ...
        if role == "certifier":
            ...
            synthetic = base._delete_synthetic_candidates(db, schema)
            replication = base._delete_acknowledged_replication(
                db,
                schema,
            ...
            stale_measurements = base._delete_stale_measurements(db, schema, cutoff=cutoff)
        ...
        protected_after = _protection_snapshot(db, schema_after, plan)
...
def main(argv: list[str] | None = None) -> int:
    # Keep a CLI for local/ops parity while production uses the guarded runtime owner.
    import argparse
    import sys

    parser = argparse.ArgumentParser(description="Fail-closed one-shot production SQLite cleanup v3")
