...
from .shadow_price_tracking_state_repair import install_shadow_price_tracking_state_repair
...
# second. Install a durable recent-mint state before production composition so the
# clock reconstructs exact state through bounded rowid-keyset batches, then serves
# steady-state ticks from a small indexed table. No historical evidence is deleted,
# no startup full-history index build is introduced, and the clock remains fail-closed
# until its exact bounded state is ready.
