...
# The full-scope dispatcher now amortizes all launch/scout/bootstrap/sample receipt
# writes into one bounded transaction. Keep those semantics, but move that exact
# synchronous persistent-disk transaction off the single Uvicorn event-loop thread
# so Render's five-second liveness probe is not delayed by SQLite/fsync work.
