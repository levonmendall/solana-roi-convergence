...
async def _buy_with_modern_admission(self: Any, row: dict[str, Any]) -> None:
    ...
    # Preserve the exact existing path for already-copyable observations.
    ...
    # The compatibility bit is changed only in this ephemeral call copy. Durable
    # wallet evidence remains truthful (copyable=0), while v5/v5.1 gets the chance
    # to obtain its own amount-specific executable entry/exit evidence and then
    # enforce its actual <=20s, >40% observe-only and mechanical-stop rules.
    admitted = dict(row)
    admitted["copyable"] = 1
    setattr(
        self,
...
def _market_regime_with_modern_admission(self: Any, at: datetime) -> MarketRegime:
    ...
    # The only legacy-copyability-dependent regime branch is high_speculation.
    # Preserve weak/broad-mania precedence and all numeric regime thresholds, but
    # allow timely forward buys to represent speculative flow even when the old
    # <=15% mark bit is false. Final execution remains independently fail-closed.
