...
def _decision_cutoff(swaps: list[Any], explicit: float | None = None) -> float:
    ...
    if value is not None and value > 0.0:
        return value
    # recent_swaps is append ordered in the production chain plane.  Using the
    # event that triggered the decision makes replay/out-of-order processing
    # point-in-time correct: an older event appended later cannot see already
    # buffered future activity.
    ...
    *,
    ...
    for raw in items:
        ...
        if age is None or age < 0.0:
            # Explicitly reject future evidence.  Do not depend on list/ingest
            # order to enforce the point-in-time boundary.
            continue
        if age <= 60.0:
            current.append(raw)
        elif age <= 120.0:
...
def managed_position_nav_multiplier(rows: Iterable[Any]) -> float:
    ...
    Each exit event already expresses its realized portfolio fraction and the
    return on the sold cost basis.  Those contributions are additive within one
    canonical position.  Separate positions retain multiplicative sequencing,
    matching the pre-existing paper-NAV convention without double compounding
    staged exits from the same capital commitment.
    """
