...
def test_epoch_anchor_preserves_historical_cursor_and_verifies_factory_gap(monkeypatch) -> None:
    ...
    asyncio.run(repair._advance_live_epoch(plane))
...
def test_large_live_gap_reanchors_without_replaying_it_as_current(monkeypatch) -> None:
    synced: list[tuple[int, int]] = []
...
def test_status_exposes_live_readiness_separately_from_historical_backfill() -> None:
    class Plane:
        _cursor = 1000
        _latest_block = 2001
        _caught_up = False
    ...
    assert payload["historical_block_lag"] == 1001
    assert payload["caught_up_for_paper_decisions"] is True
    assert payload["paper_decision_transport_ready"] is True
    assert payload["catchup_capacity"]["paper_entries_allowed_during_catchup"] is True
    assert payload["live_frontier_verification"]["retrospective_entry_authority"] is False
    assert payload["live_frontier_verification"]["historical_backfill_can_authorize_entries"] is False
