...
def test_production_installs_explicit_final_v52_authority_and_v51_control_routes() -> None:
    ...
    assert str(production.app.state.roi_v52_economic_composition).startswith("v52-explicit-production-authority-")
    assert production.app.state.roi_authority_id == "roi-convergence-v5.2-authoritative-1"
    assert production.app.state.roi_authoritative_strategy_version == "roi-convergence-v5.2-continuation-capture-1"
    paths = {getattr(route, "path", None) for route in production.app.routes}
    assert "/v1/strategy/authority" in paths
    assert "/v1/strategy/v52" in paths
    assert "/v1/strategy/control/v51-authority" in paths
    # v5.1 proof/measurement surfaces remain reachable as read-only evidence.
...
def test_final_strategy_functions_are_marked_as_v52_authoritative() -> None:
    from solana_roi import production  # noqa: F401
