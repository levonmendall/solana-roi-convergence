...
def test_out_of_order_late_event_uses_event_time_not_ingest_order_or_wall_clock() -> None:
    # A future event is already buffered, then an older historical event arrives
    # last.  The older event is the decision boundary and may not see the future.
    swaps = [
        _swap(990.0, A),
...
def test_late_historical_evidence_before_cutoff_is_preserved() -> None:
    ...
    metrics = asyncio.run(
        repair._point_in_time_flow_metrics(
            _Entities(), swaps, decision_cutoff_ts=1000.0
        )
    )
    assert metrics["buy_count_60s"] == 3
...
def test_staged_exit_contributions_are_additive_within_one_position() -> None:
    ...
    # This is the prior bug: compounding the two slices independently.
...
def test_production_import_installs_integrity_wrappers_without_live_money_authority() -> None:
    # Importing v52_strategy_api is the production-authority import-time boundary
    # used to install this repair before lifecycle method capture.
