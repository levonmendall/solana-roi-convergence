...
def test_reinstall_after_e2e_outer_wrapper_does_not_create_worker_cycle(monkeypatch) -> None:
    def deep_builder():
        return {"state": "INSUFFICIENT_EVIDENCE"}

    app, _other_route, _proof_route, _other = _fake_app(deep_builder)

    async def e2e_outer(runtime, stop):
        return None

    setattr(e2e_outer, "_roi_e2e_status_snapshot_worker", True)
    monkeypatch.setattr(e2e, "_ORIGINAL_RUNTIME_WORKERS", repair._runtime_workers_with_production_proof_snapshot)
    monkeypatch.setattr(render_bootstrap, "_run_runtime_workers", e2e_outer)

    assert repair._current_worker_chain_already_contains_production_proof(e2e_outer) is True
    repair.install_production_proof_read_boundary_repair(app)

    # The existing outer E2E worker remains the root. Re-wrapping it with the proof
    # worker would form E2E -> proof -> E2E and recurse indefinitely.
    assert render_bootstrap._run_runtime_workers is e2e_outer
    assert getattr(e2e_outer, "_roi_production_proof_snapshot_worker", False) is True


def test_production_composition_owns_both_nonblocking_proof_boundaries_and_continuity() -> None:
    from pathlib import Path
    from solana_roi import production_system

    source = Path(production_system.__file__).read_text(encoding="utf-8")
    assert "install_same_release_continuity_epoch_repair()" in source
    ...
    assert "same-release-continuity-successor" in source
    assert "production-proof-read-boundary" in source
