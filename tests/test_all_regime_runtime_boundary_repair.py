...
def test_render_blueprint_declares_canonical_blockscout_secret() -> None:
    assert "- key: BLOCKSCOUT_API_KEY\n        sync: false" in Path("render.yaml").read_text()


def test_robinhood_transport_readiness_fails_closed_on_stale_caught_up_flag() -> None:
    assert repair._paper_transport_ready(SimpleNamespace(_caught_up=True, _cursor=100, _latest_block=200)) is False
...
async def test_dense_scout_gap_can_cross_old_three_page_boundary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def page(start: int, count: int) -> list[dict[str, object]]:
        return [{"signature": f"sig-{slot}", "slot": slot} for slot in range(start, start - count, -1)]

    pages = [page(4100, limit), page(3100, limit), page(2100, limit), page(1100, limit), page(100, 11)]
