...
def test_durable_background_batch_runs_through_asyncio_worker_thread(monkeypatch):
...
def test_install_replaces_capacity_dispatch_worker_and_marks_status():
    ...
    assert raw_dispatch._dispatch_worker is repair._web_safe_capacity_dispatch_worker
...
def test_status_reports_no_durability_or_scope_weakening():
    ...
    assert status["bulk_receipt_sqlite_off_event_loop"] is True
    ...
    assert payload["raw_receipt_dispatch"]["bulk_sqlite_commit_off_uvicorn_event_loop"] is True
