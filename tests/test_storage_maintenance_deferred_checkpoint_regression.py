...
def _run_one_worker_pass(monkeypatch, *, monotonic_values: list[float]) -> tuple[list[float], list[str]]:
    stop = asyncio.Event()
    ...
    monkeypatch.setattr(repair.time, "monotonic", lambda: next(monotonic))
...
def test_storage_maintenance_checkpoint_remains_enabled_after_interval(monkeypatch) -> None:
    interval = storage_capacity.WAL_CHECKPOINT_INTERVAL_SECONDS
    observed_timeouts, checkpoint_calls = _run_one_worker_pass(
        monkeypatch,
        monotonic_values=[100.0, 100.0 + interval + 0.1],
    )
