...
def test_actual_scout_normalizer_boundary_schedules_unpriced_probe(monkeypatch) -> None:
    ...
    assert plane._roi_post182_wiring_unpriced_probe_candidates_seen == 1
    assert plane._roi_post182_wiring_probe_schedule_calls == 1


def test_normalizer_does_not_schedule_without_durable_unpriced_economic_row(monkeypatch) -> None:
    plane = SimpleNamespace()
    scheduled: list[str] = []
...
def test_real_pump_websocket_publishes_only_after_durable_sqlite_commit(tmp_path, monkeypatch) -> None:
    ...
    assert plane._roi_post182_wiring_real_pump_ws_seen == 1
    assert plane._roi_post182_wiring_ws_frontier_published == 1
...
def test_live_poll_can_never_publish_exact_websocket_frontier(tmp_path, monkeypatch) -> None:
    ...
    assert int(getattr(plane, "_roi_post182_wiring_real_pump_ws_seen", 0) or 0) == 0
...
def test_real_websocket_without_durable_row_fails_closed(tmp_path, monkeypatch) -> None:
    ...
    assert int(getattr(plane, "_roi_post182_wiring_ws_frontier_published", 0) or 0) == 0
    assert plane._roi_post182_wiring_ws_durable_not_yet_committed == 1
...
def test_existing_socket_read_context_is_preserved(tmp_path, monkeypatch) -> None:
    ...
    assert plane._roi_post182_wiring_ws_context_preserved == 1
