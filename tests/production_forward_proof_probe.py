...
SAMPLES = int(os.getenv("LIVE_MEMORY_SAMPLES", "32"))
SLEEP_SECONDS = float(os.getenv("LIVE_MEMORY_SAMPLE_SECONDS", "4"))
...
def get(path: str) -> dict:
    request = urllib.request.Request(
        f"{BASE_URL}{path}",
        headers={"Accept": "application/json", "User-Agent": "solana-roi-memory-forensics-readonly/1"},
    )
    ...
    for number in range(1, SAMPLES + 1):
        ...
        output = {
            ...
            "repair": {key: repair.get(key) for key in (
                "proof_generations", "bounded_record_builds", "fomo_outcome_rows_read",
                "promotion_cache_hits", "promotion_cache_misses", "resource_guard_relaxed",
